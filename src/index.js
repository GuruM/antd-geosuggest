...
export default class AntdGeosuggest extends React.Component<Props, State> {
  ...
  componentDidUpdate() {
    const { multiple, hasClearButton } = this.props;
  ...
  handleChange = (value: Options) => {
    ...
    Promise.all(
      value.map((site) =>
        promiseGeocode({ placeId: site.key, address: site.label })
      )
    ).then((result) => {
  ...
  render() {
    ...
    const { className, placeholder, selectStyle, hasClearButton } = this.props;

    const defaultSelectStyle = { width: "100%" };

    if (hasClearButton) {
      defaultSelectStyle.width = "80%";
      defaultSelectStyle.marginRight = "10px";
    }
    ...
    return (
      ...
      >
        <Select
          ...
          notFoundContent={
            !disabled && (fetching ? <Spin size="small" /> : "No result")
          }
          filterOption={false}
        ...
        >
          {data.map((d) => (
            <Option key={d.placeId}>{d.address}</Option>
          ))}
        </Select>
        {hasClearButton && <Button onClick={this.clearValue}>Clear</Button>}
      </div>
    );
  }
}
